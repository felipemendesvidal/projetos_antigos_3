<%@ Page Title="Painel de Controle - CAS" Language="C#" MasterPageFile="~/Pages/Admin/Admin.Master"
    AutoEventWireup="true" EnableEventValidation="false" CodeBehind="ListaConteudo.aspx.cs"
    Inherits="SolutSoft.CAS.Pages.Admin.ListaConteudo" %>
        ...
        <tr>
            <td>
                <asp:GridView ID="gvConteudo" runat="server" AutoGenerateColumns="False" OnPageIndexChanging="gvConteudo_PageIndexChanging"
                    ...
                    <Columns>
                        <asp:TemplateField ShowHeader="true" ControlStyle-CssClass="row" HeaderStyle-HorizontalAlign="Left" ItemStyle-Width="97%"
                            ItemStyle-HorizontalAlign="Right">
                            <HeaderTemplate>
                                <asp:Label ID="lblTitulo" runat="server" CssClass="titlegrid" Width="100%" Text="Título"></asp:Label>
                            </HeaderTemplate>
                            <ItemTemplate>
                                <asp:LinkButton ID="lkbEditar" runat="server" Width="100%" CommandArgument='<%# Eval("IdConteudo") %>'
                                    Text='<%# Eval("DsTitulo") %>' OnClick="lkbEditar_Click" />
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                    <Columns>
                        <asp:TemplateField ShowHeader="true" ControlStyle-CssClass="row" HeaderStyle-HorizontalAlign="Left"
                            ...
                            <ItemTemplate>
                                <asp:ImageButton ID="imbExcluir" runat="server" ImageAlign="Bottom" ImageUrl="~/Pages/Admin/Images/Icones/icone_fechar.png"
                                    CommandArgument='<%# Eval("IdConteudo") %>' OnClick="imbExcluir_Click" OnClientClick="return confirm('Quer realmente excluir este Conteúdo?');" />
                            </ItemTemplate>
                        </asp:TemplateField>
                    </Columns>
                    <EmptyDataTemplate>
